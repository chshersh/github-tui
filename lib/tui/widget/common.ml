let fmt_error (error : Gh.Client.error) (icons : Pretty.Icon.t) =
  ...
  | Bad_credentials { msg; doc_url; code } ->
      [
        str (Format.sprintf "%s [%d] %s" icons.warning code msg);
        str "";
  ...
  | Curl_error { code; msg } ->
      [
        str
          (Format.sprintf "%s GitHub API returned error code: %d" icons.warning
             code);
        str "";
        str msg;
      ]
